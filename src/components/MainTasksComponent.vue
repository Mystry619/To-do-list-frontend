...
export default {
  ...
  methods: {
    async createTask(task) {
      try {
        const response = await fetch(`https://backedn-to-do.ey.r.appspot.com/create/${this.$route.params.email}`, {
          method: 'POST',
          headers: {
    ...
    async getAllTasks() {
      try {
        const response = await fetch(`https://backedn-to-do.ey.r.appspot.com/tasks/${this.$route.params.email}`);
    ...
    async deleteTask(taskId) {
      try {
        const response = await fetch(`https://backedn-to-do.ey.r.appspot.com/tasks/${this.$route.params.email}/${taskId}`, {
          method: 'DELETE'
        });
        if (response.ok) {
          this.getAllTasks();
